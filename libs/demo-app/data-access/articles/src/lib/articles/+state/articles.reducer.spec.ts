...
import { ArticlesState, initialArticlesState, articlesReducer } from './articles.reducer';
...
describe('Articles Reducer', () => {
  ...
  describe('valid Articles actions', () => {
    it('loadArticlesSuccess should return the list of known Articles', () => {
      const articles = [createArticlesEntity('PRODUCT-AAA'), createArticlesEntity('PRODUCT-zzz')];
      const action = ArticlesActions.loadArticlesSuccess({ articles });

      const result: ArticlesState = articlesReducer(initialArticlesState, action);
